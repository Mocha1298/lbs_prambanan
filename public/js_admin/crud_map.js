...
if(input == "kecamatan"){
    ...
    $.getJSON("/dataobjek", function (data){
        ...
        var mymap = [];
        mymap[id] = L.map("mapid"+data[i].id, {
            center: [data[i].lintang,data[i].bujur],
            zoom: 20,
            // scrollWheelZoom: false,
        });
        L.tileLayer('http://{s}.google.com/vt/lyrs=s,h&x={x}&y={y}&z={z}',{
        ...
        L.marker([data[i].lintang,data[i].bujur],{icon:current})
        .addTo(mymap[id])
        .bindPopup("Lokasi terkini");
        }
    })
    ...
    for (let i = 0; i < classname.length; i++) {
        id = $(classname[i]).attr('id');
        var url = "/datapeta_kr/"+id;
        $.getJSON(url, function (data3){
            document.getElementById("bujur"+data3.id).value = data3.bujur;
            document.getElementById("lintang"+data3.id).value = data3.lintang;
            mymup[i] = L.map("mapid"+data3.id, {
